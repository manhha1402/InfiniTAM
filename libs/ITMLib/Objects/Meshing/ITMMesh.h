...
public:
  ...
  {
    std::cout<<"MaxTriangles: "<<maxTriangles<<std::endl;
  ...
  void getMesh(std::vector<Eigen::Vector3d>& vertices, std::vector<Eigen::Vector3i>& faces)
  {
    ...
    {
      vertices.push_back(Eigen::Vector3d(triangleArray[i].p0.x,triangleArray[i].p0.y,triangleArray[i].p0.z));
      vertices.push_back(Eigen::Vector3d(triangleArray[i].p1.x,triangleArray[i].p1.y,triangleArray[i].p1.z));
      vertices.push_back(Eigen::Vector3d(triangleArray[i].p2.x,triangleArray[i].p2.y,triangleArray[i].p2.z));
    }
    for (size_t i = 0; i<noTotalTriangles; i++)
    {
      faces.push_back(Eigen::Vector3i(i * 3 + 2 + 1,i * 3 + 1 + 1,i * 3 + 0 + 1));
    }
